...
const QuizLeaderboard = ({ quizId, className = '', onDataLoad }) => {
  ...
  const fetchLeaderboard = async (showLoading = true, page = currentPage) => {
    try {
      ...
      const response = currentUser 
        ? await quizAPI.getQuizLeaderboard(quizId, params)
        : await quizAPI.getPublicQuizLeaderboard(quizId, params);
      
      // Handle both old format (array) and new format (object with pagination)
      let leaderboardData, pagination, userPos;
      ...
      if (Array.isArray(response.data.data)) {
        ...
        userPos = response.data.data.userPosition || null;
      }
      
      setLeaderboard(leaderboardData);
      setUserPosition(userPos);
      setTotalEntries(pagination.totalEntries || leaderboardData.length);
  ...
  const isUserInDisplayed = currentUser && leaderboard.some(entry => entry.user._id === currentUser._id);
    ...
    return (
      <div className={`bg-white rounded-xl shadow-lg p-3 sm:p-6 ${className}`}>
        <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-3 sm:mb-4 flex items-center gap-2">
          ...
          <span className="hidden sm:inline">Quiz Leaderboard</span>
    ...
    <div className={containerClasses}>
      ...
      {leaderboard.length === 0 ? (
        <div className="bg-gray-50 rounded-xl p-4 sm:p-6 border border-gray-200 text-center">
          ...
          <p className="text-gray-500 text-xs sm:text-sm">Be the first to attempt this quiz!</p>
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs sm:text-sm text-gray-600 flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                    <span className="flex items-center gap-1">
                      ...
                      {userPosition.totalAttempts} attempts
                    </span>
                  </div>
                </div>
                <div className="text-right flex-shrink-0">
                  <div className="text-lg sm:text-2xl font-bold text-[#00bcd4]">{userPosition.bestPercentage?.toFixed(1)}%</div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="bg-gray-50 px-3 sm:px-6 py-2 sm:py-3 border-t border-gray-200">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-1">
                <div className="text-xs sm:text-sm text-gray-600">
                  <span className="hidden sm:inline">Rankings based on best score and attempt count</span>
                  <span className="sm:hidden">Rankings by best score</span>
                </div>
